                        ...
                        if(!e_doc[ip_s][i][events_list[j]].isNull()) {
                            if(!e_doc[ip_s][i][events_list[j]] && events_subscriptionSchema[j]) {
                                ...
                                while(!parsingDone && k<es_doc[events_list[j]]["subscription"].size()) {
                                    ...
                                    t_name.replace("\"", "");
                                    Serial.printf("Name received in subscrpiton: %s\n", t_name);
                                    if(!resp_doc[t_name].isNull()) {
                                        tmp[t_name] = es_doc[events_list[j]]["subscription"][k]["value"];
                                        String value = "";
                                        serializeJson(es_doc[events_list[j]]["subscription"][k]["value"], value);
                                        Serial.printf("Subscription value: %s\n", value);
                                        validInput++;
                                    }
                                    else  
                                        parsingDone = false;
                                ...
                                if(validInput == k) {
                                    ...
                                    if(validate.equals(message)) {
                                        ...
                                        Serial.print("Refused. tmp: ");
                                        serializeJson(tmp, Serial);
                                        Serial.println();
                                        Serial.print("Refused. resp_doc: ");
